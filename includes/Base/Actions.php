...
class Actions
{
    ...
    {
        add_action( 'wp_ajax_sync_old_media', [$this, 'syncOldUploads'] );
        add_action( 'init', [$this, 'scheduleSyncOldUploads'] );
    }

    public function scheduleSyncOldUploads() : void
    {
        // Schedule the cron event to run every 20 minutes
        if ( !wp_next_scheduled('sync_old_uploads_cron') ) {
            wp_schedule_event( time(), 'every_20_minutes', 'sync_old_uploads_cron', ['cron' => true] );
        }

        // Hook the cron event to the syncOldUploads function
        add_action( 'sync_old_uploads_cron', [$this, 'syncOldUploads'] );
    }
     ...
     * @return void
     */
    public function syncOldUploads( $args ) : void
    {
        // Retrieve the cron argument
        $cron = $args['cron'] ?? false;
        ...
        $progress = get_option( 's3_media_handler_sync_old_uploads_progress', [] );
        $batchSize = 20;

        // Retrieve all the old uploads from the database
        $oldAttachments = get_posts([
